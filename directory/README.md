...
boot|부팅에 필요한 가진 파일들이 저장
dev|시스템 디바이스 관련 파일들이 저장
etc|시스템 설정에 관련된 각종 파일들이 저장
home|사용자의 홈 디렉토리가 생성
lib|커널과 프로그램에 필요한 각종 라이브러리가 저장
media|CD, USB같은 외부 장치를 연결
mnt|탈부착 가능한 장치들을 임시로 연결| (WSL의 경우 윈도우의 디렉토리와 연결)
opt|추가 패키지가 설치
root|root(최고관리자)계정의 홈 디렉토리
run|실행중인 서비스와 관련된 파일들이 저장
sbin|시스템 관리자용 명령어들이 저장
sys|리눅스 커널 관련 정보들이 저장
tmp|시스템 사용중 발생하는 임시데이터가 저장
usr|기본 실행파일, 라이브러리, 헤더 파일등이 저장
var|시스템 운영중 발생하는 데이터, 로그가 저장
...
리눅스 시스템의 디렉토리 구조는 전체적으로 역 트리(tree) 구조이다. 

또한 명령어의 종류와 성격, 사용권한등에 따라 각각의 디렉토리들로 구분된다.   
